...
public class AgentTaskService {
        ...
        public AgentTaskService(AgentRepository agentRepository,
        ...
        @Transactional
        public AgentTaskResponse createTask(CreateAgentTaskRequest request) {
                Agent agent = agentRepository.findById(request.agentId())
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Agent not found. agentId=" + request.agentId()));

                AgentTask task = AgentTask.create(request.agentId(), request.taskType(),
        ...
        public NextAgentTaskResponse nextTask(Long agentId, String agentToken) {
                getAgentAndValidateToken(agentId, agentToken);

                return taskRepository
                                .findTop1ByAgentIdAndStatusOrderByCreatedAtAsc(agentId,
                                                AgentTaskStatus.QUEUED)
                                .stream().findFirst().map(NextAgentTaskResponse::from)
        ...
        public AgentTaskResponse startTask(Long agentId, Long taskId,
                ...
                AgentTask task = getTaskOwnedByAgent(agentId, taskId);
        ...
        @Transactional
        public AgentTaskResponse failTask(Long agentId, Long taskId, FailAgentTaskRequest request) {
                getAgentAndValidateToken(agentId, request.agentToken());
        ...
        private AgentTask getTaskOwnedByAgent(Long agentId, Long taskId) {
                AgentTask task = taskRepository.findById(taskId)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Agent task not found. taskId=" + taskId));

                if (!agentId.equals(task.getAgentId())) {
                        throw new IllegalStateException("Task does not belong to agent. agentId="
        ...
        @Transactional
        public AgentTaskResponse createBackupTaskForOperationJob(Long operationJobId,
                        Long databaseId) {
                ...
                AgentTask task = AgentTask.createForOperationJob(agent.getId(), operationJobId,
                                AgentTaskType.MYSQL_LOGICAL_BACKUP, parametersJson);
