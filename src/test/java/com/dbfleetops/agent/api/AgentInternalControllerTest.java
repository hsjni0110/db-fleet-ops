...
class AgentInternalControllerTest {
        ...
        void registerReturnsAgentIdAndToken() throws Exception {
                when(agentService.register(any())).thenReturn(new RegisterAgentResponse(1L,
                                "agent-token-001", AgentStatus.ONLINE));
        ...
        void heartbeatReturnsOnlineStatus() throws Exception {
                when(agentService.heartbeat(eq(1L), any())).thenReturn(new AgentHeartbeatResponse(
                                1L, AgentStatus.ONLINE, LocalDateTime.of(2026, 7, 4, 10, 0)));
